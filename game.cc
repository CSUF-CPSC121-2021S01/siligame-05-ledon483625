...
void Game::UpdateScreen() {
  ...
  if (!HasLost()) {
    game_screen_.DrawText(0, 0, std::to_string(score_), 25,
                          graphics::Color(0, 0, 255));
  } else {
    game_screen_.DrawText(game_screen_.GetWidth() / 2 - 225,
                          game_screen_.GetHeight() / 2, "VACCINATION FAILED",
                          50, graphics::Color(255, 0, 0));
  }
}
